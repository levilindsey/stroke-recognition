...
namespace StrokeCollector
{
	public class Recognizer
	{
		...
		public class PairComparer : IEqualityComparer<Tuple<float, float>>
		{
			public bool Equals(Tuple<float, float> x, Tuple<float, float> y)
			{
			...
			public int GetHashCode(Tuple<float, float> x)
			{
				if (x == null)
				{
					return 0;
