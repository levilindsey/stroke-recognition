...
namespace StrokeCollector
{
	class StrokePreProcessing
	{
		...
		{
			return GetAngle(previousPoint.X, previousPoint.Y, nextPoint.X, 
				nextPoint.Y, enforcePositivity);
		}

		/// <summary>
		/// Return the angle (in radians) between the two given points.
		/// </summary>
		public static double GetAngle(double previousPointX,
			double previousPointY, double nextPointX, double nextPointY,
			bool enforcePositivity)
		{
			double deltaX = nextPointX - previousPointX;
			double deltaY = nextPointY - previousPointY;
